...
旧分類の `Research` は、compatibility contract以降は独立ラベルとして使わない。OpenTaiko調査が未完の項目は、一時的に `Must parse / must not crash` へ置き、OpenTaiko researchの採用判断で確定させる。
...
| F-017 | 複素SCROLL | Must parse / must not crash | parserで認識し、OpenTaiko researchで数値表現を確定 | compatibility report |
