...
Phase1スコープ定義の受け入れ基準ではbinary名が暫定だった。Rustアーキテクチャ方針で次に固定する。
...
Timing / Audio / Judgement検証設計以降のコマンド例は `taiko_play`、`headless_autoplay`、`timing_log_analyzer` を正とする。
...
### 5.1 Dependency selection policy

Phase1では、次の外部crateを依存関係の選定対象として固定する。実装開始時に `Cargo.lock` で正確なversionを固定する。
...
autonomy scorecard時点の正規コマンド名は次である。
...
## OpenTaiko compatibility responsibilities

OpenTaiko researchで、Phase1は単純な通常play loopではなく、OpenTaiko通常プレイ互換の中核機能を扱う方針に更新された。Architectureは次を正式責務として持つ。

| crate | OpenTaiko researchで追加された責務 |
|---|---|
