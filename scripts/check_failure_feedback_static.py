#!/usr/bin/env python3
"""Static validation for Step11 Failure Feedback Loop MVP."""
...
CHECKS = [
    ...
    ("docs/40_loop_cli_contract.md", ["Step11 failure feedback command surface"]),
...
def main() -> int:
    ...
    for rel in REQUIRED_FILES:
        if not (ROOT / rel).is_file():
            failures.append(f"missing required Step11 file: {rel}")
    ...
    if bootstrap.is_file():
        ...
        for term in ["docs/48_failure_feedback_loop_contract.md", "scripts/check_failure_feedback_static.py", "GATE-0070-failure-feedback-ready.md"]:
            if term not in text:
                failures.append(f"bootstrap consistency script missing Step11 term: {term}")
    if failures:
